...
use crate::action::{git_fetch, Handler, scan};
use crate::action::models::{GITHUB_ADD_COMMENT, GITHUB_DELETE_COMMENT, GITHUB_QUERY, GithubPayload, GithubPullRequestPayload, SLOGAN};
...
impl Github {
    pub fn new(project: &str, repository: &str, id: u32) -> Github {
        ...
        header.insert(reqwest::header::ACCEPT, reqwest::header::HeaderValue::from_static("application/vnd.github+json"));
    ...
    fn get_pull_request(&self) -> anyhow::Result<PullRequest> {
        ...
        loop {
            ...
            let resp: Response = self.client.post("https://api.github.com/graphql")
            ...
            if resp.status().is_success() {
                ...
                data.repository.pull_request.files.edges
                    .iter()
                    .map(|x| x.node.path.deref())
                    .for_each(|x| files.push(x.to_string()));
                data.repository.pull_request.comments.edges
                    .iter()
                    .map(|x| Comment{id: x.node.id.to_string(), body: x.node.body.to_string()} )
                    .for_each(|x| comments.push(x));
                if !data.repository.pull_request.files.page_info.has_next_page && !data.repository.pull_request.comments.page_info.has_next_page {
                    break;
                }
        ...
        let comment_ptr = comments.iter().find(|x| {
            x.body == SLOGAN
        });
...
impl Handler for Github {
    fn execute(&mut self) -> anyhow::Result<()> {
        ...
        git_fetch(&pull_request.files, &format!("https://github.com/{project}/{repo}.git", project=self.project, repo=self.repository), self.id)?;
        let yes = scan()?;
